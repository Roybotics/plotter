...
class ApollonianGasket(object):
    ...
    def recurse(self, circles, depth, maxDepth, minRadius):
        ...
        if( depth == 0 ):
            ...
            self.recurse( (cspecial, c2, c3, c4), 1, maxDepth, minRadius )

        cn2 = secondSolution( c2, c1, c3, c4 )
        if cn2.r.real>=minRadius:
            self.genCircles.append(cn2)
            self.recurse( (cn2, c1, c3, c4), depth+1, maxDepth, minRadius )
        
        cn3 = secondSolution( c3, c1, c2, c4 )
        if cn3.r.real>=minRadius:
            self.genCircles.append(cn3)
            self.recurse( (cn3, c1, c2, c4), depth+1, maxDepth, minRadius )
        
        cn4 = secondSolution( c4, c1, c2, c3 )
        if cn4.r.real>=minRadius:    
            self.genCircles.append(cn4) 
            self.recurse( (cn4, c1, c2, c3), depth+1, maxDepth, minRadius )



    def generate(self, depth, minRadius = 0.02):
        """
        Wrapper for the recurse function. Generate the AG,
        @param depth: Recursion depth of the Gasket
        @type depth: int
        """
        self.recurse(self.start, 0, depth, minRadius)
